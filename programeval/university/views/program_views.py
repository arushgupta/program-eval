...
from university.models import Program, ProgramCourse, ProgramCourseObjective, ProgramObjective, Section, SectionSubObjective, SubObjective
        ...
        if form.is_valid():
            ...
            if pco.sub_objective:
                for section in sections:
                    section_sub_objectives.append(SectionSubObjective(section=section, program_course_objective=pco, program_course=pco.program_course, 
                                                                      program_objective=pco.program_objective, sub_objective=pco.sub_objective))
                ...
                for section in sections:
                    for sub_objective in sub_objectives:
                        section_sub_objectives.append(SectionSubObjective(section=section, program_course_objective=pco, program_course=pco.program_course, 
                                                                          program_objective=pco.program_objective, sub_objective=sub_objective))
                SectionSubObjective.objects.bulk_create(section_sub_objectives)
            
            else:
                for section in sections:
                    section_sub_objectives.append(SectionSubObjective(section=section, program_course_objective=pco, program_course=pco.program_course, 
                                                                      program_objective=pco.program_objective, sub_objective=None))
                SectionSubObjective.objects.bulk_create(section_sub_objectives)
            return redirect('program-detail', program_id)
    else:
